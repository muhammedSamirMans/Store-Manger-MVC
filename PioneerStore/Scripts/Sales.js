$(document).ready(function () {
    ...
    $("#StoreName").change(function () {
        ...
        $.ajax({
            type: "post",
            url: "../Sales/GetItemsList/" + storeID,
            contentType: "html",
    ...
    $("#UnitName").change(function () {
        ...
        $.ajax({
            url: "../Sales/GetItembyID/" + $("#ItemName").val(),
            type: "GET",
...
$("body").on("click", "#btnAdd", function () {
    ...
    item.Total = 0;
    $.ajax({
        url: "../Sales/GetAvaliableQuantity/",
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $("#tblSkills > TBODY TR").each(function () {
                            var row = $(this);
                            if ($("#StoreName").val() == row.find("TD").eq(0).html() && $("#ItemName").val() == row.find("TD").eq(1).html()) {   //get the new value of quantity the old value+ new value
                                var value = parseFloat(row.find("TD").eq(2).html()) + parseFloat($("#Quantity").val() * parseFloat(unitName.val()));
                                //get the price value
                                //what if the item quantity store is less than the new quantity

                                if (value > currentQuantity) {
                                    alert("we don't have the orederd quantity(" + value + ") sorry! we just have :" + currentQuantity);
                                }
                                else {
                                    var price = parseFloat(row.find("TD").eq(3).html());
                                    //calc addedtax = (the price *new quantity)*(addedtax/100).
                                    var addedtax = (price * value * ((data.AddedTax / 100)));
                                    var total = ((price * value) + addedtax);
                                    alert("this item in the items table! the new quanitity= " + value);
                                    //create new row contianing the new values
                                    var newrow = '<tr class="new_row"><td>' + $("#StoreName").val() + '</td><td>' + $("#ItemName").val() + '</td><td>' + value + '</td><td>' + price + '</td><td>' + addedtax + '</td><td>' + total + '</td><td>' + '<input value="x" class="btn btn-danger btn-sm" onclick="Remove(this);" type="button" />' + '</td></tr>';
                                    //repleace the old row with the new one
                                    $(this).replaceWith(newrow);
                                    //update total and remaining text
                                    var total = 0;
                                    $("#tblSkills  TBODY TR").each(function () {
                                        var row = $(this);
                        ...
                        //if the no repeated Item in same store will create anew row and add it to table
                        if (foundflag == false) {
                            if (unitName.val() * quantity.val() > currentQuantity) {
                                alert("sorry we don't have enough quantity ,we just have:" + currentQuantity);
                            }
...
$("body").on("click", "#btnSave", function () {
    ...
    if ( $("#Client").val() && parseFloat($("#TTotal").text()) && $("#Payed").val() && (finalremaning || finalremaning == 0 || parseFloat($("#Remaining").text()) > 0) && ($("#Payed").val() < finaltotal || $("#Payed").val() == finaltotal) && ($("#Payed").val() == 0 || $("#Payed").val() > 0)) {
